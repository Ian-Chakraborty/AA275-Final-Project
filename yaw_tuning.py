...
gains = np.array([0,0,0,0,0,00,3.7215,1.1307])
...
logger.write_log("Starting 60s tracking")  
...
while time.time() < t_end:
    ...
    psi_rel_deg_last = psi_rel_deg
    if goal_xyz_D is not None: 
        send_control_msgs(vehicle, logger, goal_xyz_D, psi_rel_deg, gains,send_command = True)
