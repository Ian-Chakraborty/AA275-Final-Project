...
gains = np.array([1,0.1,0,0,1,0.1,3.7215,1.1307])
...
while time.time() < t_end:
    ...
    if goal_xyz_D is not None: 
        send_control_msgs(vehicle, logger, goal_xyz_D, psi_rel_deg, gains, send_command = False)

logger.write_log("Starting 10s tracking")  
...
while time.time() < t_end:
    ...
    if camera.target_detected == True:
        ...
        goal_xyz_D = np.array([[0],[0],[0]])
        psi_rel_deg = np.array([0])
    elif ((t - camera.time_of_prev_detection) < max_time_between_detections) and (mode == "TRACK") and (camera.target_detected == False):
